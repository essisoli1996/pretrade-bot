...
export default async function handler(req: Request): Promise<Response> {
  ...
  const rpcFor = (c: string) => {
    const one: any = async (method: string, params: unknown[]) => { const r = await http(RPCS[c], { jsonrpc: "2.0", id: 1, method, params }); return r.json ?? { error: { code: r.status, message: r.text.slice(0, 80) } }; };
    one.batch = async (calls: [string, unknown[]][]) => {
      const r = await http(RPCS[c], calls.map(([method, params], id) => ({ jsonrpc: "2.0", id, method, params })));
      if (!Array.isArray(r.json)) return null;
      const byId = new Map(r.json.map((x: any) => [x.id, x]));
