// pretrade / batch-check — self-contained x402 Cloud handler (Request → Response)
...
export default async function handler(req: Request): Promise<Response> {
  ...
  let raw: unknown = url.searchParams.get("addresses");
  let chainKey: string | null = url.searchParams.get("chain");

  if (req.method === "POST") {
    try {
      const body: any = await req.json();
      raw = body?.addresses ?? raw;
      chainKey = body?.chain ?? chainKey;
    } catch {
      return bad(400, "invalid_json", 'POST body must be JSON: {"addresses":["0x…"],"chain":"base"}');
    }
  }
  ...
  const list = (Array.isArray(raw) ? raw : String(raw ?? "").split(","))
    .map((a) => String(a).trim())
    .filter(Boolean);
  const addresses = [...new Set(list.map((a) => norm(a)))];

  if (!addresses.length) return bad(400, "missing_addresses", "Pass ?addresses=0x…,0x… or a JSON body.");
  if (addresses.length > MAX_BATCH) return bad(400, "too_many_addresses", `Max ${MAX_BATCH} per call.`);
  const invalid = addresses.filter((a) => !validAddr(a));
  if (invalid.length) return bad(400, "invalid_address", `Invalid: ${invalid.join(", ")}`);
  if (new Set(addresses.map(isSol)).size > 1) return bad(400, "mixed_chains", "One chain per call: do not mix Solana mints with 0x addresses.");
  const resolved = resolveChain(addresses[0], chainKey ? String(chainKey) : null);
  if (!resolved.chain) return bad(400, "unsupported_chain", resolved.error!);
  const chain = resolved.chain;
  chainKey = resolved.key;

  const [pairs, secs] = await Promise.all([
    fetchPairs(chain.dex, addresses),
    Promise.all(addresses.map((a) => fetchSecurity(chain.goplus, a))),
  ]);
  if (!pairs && secs.every((s) => !s)) return bad(502, "upstream_unavailable", "Data sources unreachable. You were not charged.");

  const results = addresses.map((address, i) => {
    const market = marketSnapshot(bestPair(pairs ?? [], address), pairs ?? [], address);
    const r = assess(secs[i], market);
    return {
      address,
      ...
      topFlags: r.flags.slice(0, 3).map((f) => f.code),
      liquidityUsd: market?.liquidityUsd ?? null,
      maxSellUsdAt2pctImpact: market ? exitModel(market.deepestPoolUsd).maxUsd.impact2pct : null,
    };
  });
