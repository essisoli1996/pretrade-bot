// pretrade / token-check — self-contained x402 Cloud handler (Request → Response)
...
export default async function handler(req: Request): Promise<Response> {
  ...
  if (!sec && !pairs) return bad(502, "upstream_unavailable", "Data sources unreachable. You were not charged. Retry shortly.");

  const top = bestPair(pairs ?? [], address);
  const market = marketSnapshot(top, pairs ?? [], address);
  // Robinhood Chain v4 pools: read the hook and simulate a real buy + sell on the pool
  const v4 = chainKey === "robinhood" && top ? await V4.read(top, norm(address)) : null;
  const result = V4.merge(assess(sec, market), v4?.flags ?? []);
  const holders = sec?.__solana ? num(sec.gp?.holder_count) : sec ? num(sec.holder_count) : null;
  ...
  return Response.json({
    ...
    taxes: taxesOf(sec),
    holders,
    creator: sec && !sec.__solana ? { address: sec.creator_address ?? null, honeypotHistory: yes(sec.honeypot_with_same_creator) } : null,
    market,
    exit: market ? exitModel(market.deepestPoolUsd).maxUsd : null,
    v4: v4 ? { hook: v4.hook, simulation: v4.simulation } : null,
    sources: { contractSecurity: !!sec, dexMarket: !!market, v4Simulation: v4?.simulation?.status === "ok" || ["honeypot", "cooldown", "sell-failed"].includes(v4?.simulation?.status ?? "") },
