...
export default async function handler(req: Request): Promise<Response> {
  ...
  const sellTax = taxes?.sell ?? 0;
  const risk = assess(sec, market);
  const blocked = risk.flags.some((f) => ["HONEYPOT", "CANNOT_SELL_ALL", "NON_TRANSFERABLE", "ACCOUNTS_FROZEN_BY_DEFAULT", "SELL_TAX_EXTREME"].includes(f.code));
  ...
  const advice =
    exitability === "TRAPPED" ? "Contract flags suggest selling may be impossible or punitive. Do not enter."
