// pretrade / exit-check — self-contained x402 Cloud handler (Request → Response)
...
const V4 = (() => {
  ...
  const call = (to: string, data: string, block: string, over: Record<string, unknown>) =>
    rpc("eth_call", [{ from: FROM, to, data, gas: "0x1c9c380" }, block, over]);
  async function balanceSlot(token: string, block: string): Promise<string | null> {
    const data = "0x70a08231" + addr32(SIM_ADDR);
    ...
    for (const slot of [...new Set([...keys, ...candidateSlots(SIM_ADDR)])]) {
  ...
  async function roundTrip(key: PoolKey, token: string, amount: bigint, sellOnly: boolean, block: string): Promise<Leg> {
    ...
    const over: Record<string, any> = { [SIM_ADDR]: { code: SIM_RUNTIME } };
    if (fund === ZERO) over[SIM_ADDR].balance = "0x" + amount.toString(16);
    else {
    ...
    const r = await call(SIM_ADDR, encodeRoundTrip(key, tokenIs0, sellOnly ? 0n : amount, sellOnly, sellOnly ? amount : 0n), block, over);
...
export default async function handler(req: Request): Promise<Response> {
  ...
  const usdRaw = url.searchParams.get("usd");
  ...
  if (usd !== null && (!Number.isFinite(usd) || usd <= 0 || usd > 1e9)) return bad(400, "invalid_usd", "usd must be a positive number (position size in USD).");
  ...
  const [sec, pairs] = await Promise.all([fetchSecurity(chain.goplus, norm(address)), fetchPairs(chain.dex, [address])]);
  ...
  const model = exitModel(market.deepestPoolUsd);
  const taxes = taxesOf(sec);
  const sellTax0 = taxes?.sell ?? 0;
  // Robinhood Chain v4 pools: a real buy + sell simulation beats any estimate of whether you can get out
  const v4 = chainKey === "robinhood" ? await V4.read(pair, norm(address)) : null;
  const risk = V4.merge(assess(sec, market), v4?.flags ?? []);
  const blocked = risk.flags.some((f) => ["HONEYPOT", "CANNOT_SELL_ALL", "NON_TRANSFERABLE", "ACCOUNTS_FROZEN_BY_DEFAULT", "SELL_TAX_EXTREME", "SIM_SELL_REVERTED"].includes(f.code)
    || (f.code === "SIM_ROUND_TRIP_LOSS" && f.severity === "critical"));
  // the simulation measured a whole buy+sell; the sell side is roughly the square root share of it
  const rt = (v4?.simulation as any)?.roundTripCostPct;
  const measuredSellCost = typeof rt === "number" ? 1 - Math.sqrt(Math.max(0, 1 - rt / 100)) : null;

  let position = null;
  if (usd !== null) {
    const impact = model.impact(usd);
    const sellTax = Math.max(sellTax0, measuredSellCost ?? 0);
    const totalCost = 1 - (1 - impact) * (1 - sellTax);
    const v24 = market.volume24hUsd ?? 0;
    position = {
      usd,
      estPriceImpactPct: Math.round(impact * 1000) / 10,
      sellTaxPct: Math.round(sellTax * 1000) / 10,
      sellCostSource: measuredSellCost !== null && measuredSellCost > sellTax0 ? "simulated on the live pool (fees + hook + tax)" : "contract scan",
      estTotalExitCostPct: Math.round(totalCost * 1000) / 10,
      estProceedsUsd: Math.round(usd * (1 - totalCost) * 100) / 100,
      shareOf24hVolumePct: v24 > 0 ? Math.round((usd / v24) * 1000) / 10 : null,
    };
  }

  const ref = position ? position.estTotalExitCostPct : null;
  const exitability = blocked ? "TRAPPED" : ref === null ? (market.deepestPoolUsd < 10_000 ? "THIN" : "UNSIZED") : ref <= 2 ? "EASY" : ref <= 6 ? "OK" : ref <= 15 ? "COSTLY" : "HARD";
  const advice =
    exitability === "TRAPPED" ? (v4?.simulation?.status === "honeypot" ? "A simulated sell on the live pool reverted while a control token sold fine. Do not enter."
      : typeof rt === "number" && rt >= 50 ? `A simulated buy then sell on the live pool lost ${rt}%. Do not enter.` : "Contract flags suggest selling may be impossible or punitive. Do not enter.")
    : exitability === "HARD" || exitability === "COSTLY" ? `Size is large for this pool. Keep a single sell under $${model.maxUsd.impact2pct.toLocaleString("en-US")} for ~2% impact, or split the exit.`
    : exitability === "THIN" ? "Pool is thin. Pass &usd= with your size to get a concrete estimate."
    : "Exit looks feasible at this size under current liquidity.";
  ...
  return Response.json({
    ...
    advice,
    position,
    maxSellUsd: model.maxUsd,
    sellBlockedByContract: blocked,
    simulation: v4?.simulation ?? null,
    pool: { deepestPoolUsd: market.deepestPoolUsd, totalLiquidityUsd: market.liquidityUsd, volume24hUsd: market.volume24hUsd, dex: market.topPair.dex, url: market.topPair.url },
    contractVerdict: { verdict: risk.verdict, riskScore: risk.riskScore, topFlags: risk.flags.slice(0, 3).map((f) => f.code) },
    model: "Constant-product estimate against the deepest pool. Concentrated-liquidity pools and multi-pool routing can make the real figure better or worse. Liquidity can be pulled at any time.",
